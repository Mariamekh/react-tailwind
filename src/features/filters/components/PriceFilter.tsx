  ...
  return (
    <div>
      ...
      <div className="mt-[12px] flex items-center gap-[3px]">
        ...
        <span
          aria-hidden
          className="h-[2px] w-[6px] shrink-0 rounded-full bg-ink-500"
        />
...
function NumberInput({
  ...
  return (
    <input
      ...
      className="h-10 w-[94px] rounded-lg border border-surface-border bg-white pb-[6px] pl-[10px] pr-2 pt-2 text-[14px] text-ink-800 placeholder:text-ink-500 focus:border-brand-orange"
    />
  );
}
    ...
    >
      ...
      <span className="relative z-10 flex h-full">
        <span className="flex flex-1 items-center justify-center">
          <GelGlyph color={value === 1 ? '#FFFFFF' : '#8C929B'} />
        </span>
        <span className="flex flex-1 items-center justify-center">
          <DollarGlyph color={value === 2 ? '#FFFFFF' : '#8C929B'} />
        </span>
      </span>
    </button>
  );
...
function GelGlyph({ color }: { color: string }) {
  return (
    <svg
...
function DollarGlyph({ color }: { color: string }) {
  return (
    <svg
      xmlns="http://www.w3.org/2000/svg"
      width="8"
